...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createRestaurant() {
    ...
    if (body?.operational_hours?.length > 0) {
      const create_operational_hours = body.operational_hours;
      body.operational_hours = {
        create: create_operational_hours,
      };
    } else {
      delete body.operational_hours;
    }
    if (body?.seating_layout?.length > 0) {
      const create_seating_layout = body.seating_layout;
      body.seating_layout = {
        create: create_seating_layout,
      };
    } else {
      delete body.seating_layout;
    }
